package com.company.stud.Homeworks.task5;
...
public class Task5 {/*
    public static void main(String[] args) {
        ...
        Map<String, Integer> spisok = new HashMap<>();
        for (List <String> lt : entityMap.values()) {
            for (String st : lt) {
                if (!spisok.containsKey(st)) {
                    spisok.put(st, 1);
                } else {
                    spisok.put(st, spisok.get(st) + 1);
                }
            }
        }
        spisok.forEach((k, v) -> System.out.printf("[%s] : %d\n", k, v));
